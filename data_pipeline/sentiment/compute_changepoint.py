...
Penalty:
    AIC: pen = 2  (used throughout the MERIT benchmark)
    This is less conservative than BIC (pen = log(n)) and better suited
    to detecting subtle tone shifts in short earnings call narratives,
    where managerial hedging or topic transitions produce small but
    meaningful tonal deviations.

Cost model:
    rbf (radial basis function / Gaussian kernel)
    Appropriate for bounded sentiment scores; does not assume a
    parametric distribution for the signal.
...
def load_ruptures():
    try:
        ...
        print("ERROR: ruptures is not installed. Run: pip install ruptures")
        sys.exit(1)
...
def detect_changepoints(ev_list, cp_model, cp_min_size, rpt):
    ...
    Returns a list of binary flags (1 = start of new sentiment regime).
    """
    n = len(ev_list)
    if n < cp_min_size * 2:
        return [0] * n
...
def process_file(input_path, config, cp_model, cp_min_size, dry_run, rpt, log):
    ...
    log.info(
        ...
        os.path.basename(input_path), len(rows), n_cp, rate,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Apply PELT + AIC change point detection to FinBERT-tone score files."
    )
...
def main():
    ...
    log.info("PELT + AIC: pen=%.1f, model=%s, min_size=%d",
             AIC_PENALTY, args.cp_model, args.cp_min_size)
    ...
    for input_path in files:
        n, cp = process_file(
            input_path, config, args.cp_model, args.cp_min_size,
    ...
    log.info(
        ...
        total_n, total_cp, total_cp / total_n if total_n > 0 else 0.0,
    )
